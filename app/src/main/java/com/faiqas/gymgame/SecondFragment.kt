...
class SecondFragment : Fragment(),
    ...
    private val circleRadius = 18.0f
    ...
    private val stateCallback = object : CameraDevice.StateCallback() {
        ...
        override fun onOpened(cameraDevice: CameraDevice) {
            ...
            this@SecondFragment.cameraDevice = cameraDevice
        ...
        override fun onDisconnected(cameraDevice: CameraDevice) {
            cameraOpenCloseLock.release()
            cameraDevice.close()
            this@SecondFragment.cameraDevice = null
        }

        override fun onError(cameraDevice: CameraDevice, error: Int) {
            onDisconnected(cameraDevice)
            this@SecondFragment.activity?.finish()
        }
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        soundId = soundPool!!.load(context, R.raw.up, 1)

        val sharedPref = activity?.getSharedPreferences(
            getString(R.string.gym_score_key), Context.MODE_PRIVATE
        )
        highScore = sharedPref?.getInt(getString(R.string.gym_score), 0)!!

        HighScoreTV = view.findViewById<TextView>(R.id.firsttv) as TextView
        LifeTV = view.findViewById<TextView>(R.id.thirdtv) as TextView
        ScoreTV = view.findViewById<TextView>(R.id.aanik) as TextView
        ...
        try {
            for (cameraId in manager.cameraIdList) {
                ...
                if (cameraDirection != null &&
                    cameraDirection == CameraCharacteristics.LENS_FACING_BACK //change for font camera
                ) {
                    continue
                }
    ...
    private fun setPaint() {
        ...
        paint4.textSize = 95.0f
        paint4.strokeWidth = 8.0f
    ...
    private fun draw(canvas: Canvas, person: Person, bitmap: Bitmap) {
        ...
        for (keyPoint in person.keyPoints) {
            if (keyPoint.score > minConfidence) {
                ...
                canvas.drawCircle(adjustedX, adjustedY, circleRadius, paint4)
            }
            break
        }

        var distance = person.keyPoints[1].position.x - person.keyPoints[2].position.x

        if (person.score > 0.30 && distance < 22 && distance > 10) {

            if (up) {

                LifeTV?.setText("Sit Down")
                ...
                if (person.keyPoints[0].position.y > 180) {
                    up = false
                ...
                LifeTV?.setText("Stand Up")

                if (person.keyPoints[0].position.y < 105  && !up) {
                    up = true
                    score++
                    ...
                    if (score > highScore) {
                        ...
                        with(sharedPref.edit()) {
                            putInt(getString(R.string.gym_score), score)
                            commit()
                        }
                        highScore = score
                    }
        ...
        else if (person.score < 0.10) {
            LifeTV?.text = "Where are you ?"
        }
        else {
            LifeTV?.text = "Step back"
        }

        HighScoreTV?.text = "High Score : $highScore"
        InfoTV?.text = "Carefully Stand your Smart Phone and step back. Now Push up or Sit up so that you can take yellow dot over and under the yellow line."
